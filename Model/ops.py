...
def index_points(points, idx):
    ...
    repeat_shape[0] = 1
    batch_indices = torch.array(np.arange(B, dtype=np.int32)).view(view_shape).repeat(repeat_shape)
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def execute(self, xyz1, xyz2, points1, points2):
        ...
        if S == 1:
            ...
            idx, dists = torch.argsort(dists, dim=-1)
            dists, idx = dists[:, :, :3], idx[:, :, :3]  # [B, N, 3]

            dist_recip = 1.0 / (dists + 1e-8)
            norm = torch.sum(dist_recip, dim=2, keepdims=True)
...
class FurthestPointSampler(nn.Module):
    ...
    def execute(self, x):
        ...
        idxs = torch.code(idxs_shape, 'int32', [x,], cuda_src=cuda_src)
...
class BallQueryGrouper(nn.Module):
    ...
    def execute(self, new_xyz, pointset, feature):
        ...
        ----------
        xyz: torch.Var, (B, N, 3)
        features: torch.Var, (B, N, C)

        Returns
        -------
        new_feature: torch.Var, (B, N, n_samples, C)
        '''
        ...
        if self.use_xyz:
            ...
            if new_feature is not None:
                new_feature = torch.contrib.concat([local_xyz, new_feature], dim=-1)
            else:
...
class GroupAll(nn.Module):
    ...
    def execute(self, new_xyz, pointset, feature):
        if self.use_xyz:
            new_feature = torch.contrib.concat([pointset, feature], dim=-1)
...
class KNN(nn.Module):
    ...
    def execute(self, x_q, x_r): # n_points, c_dim
        ...
        tmp_dist = torch.empty((batch_size, r_points, q_points), "float32")
        idxs,  = torch.code(
            [out_idx_shapes],
            ['int32'],
...
def topk(input, k, dim=None, largest=True, sorted=True):
    ...
    index,values = torch.argsort(input,dim=0,descending=largest)
    indices = index[:k]
    values = values[:k]
...
def index_points(points, idx):
    ...
    batch_indices = torch.array(batch_indices).view(view_shape).repeat(repeat_shape)
    new_points = points[batch_indices, idx, :]
    return new_points
...
def knn (x, k):
    inner = -2 * torch.nn.bmm(x.transpose(0, 2, 1), x)
