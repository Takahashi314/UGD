...
import numpy as np
from .ops import FurthestPointSampler
from .ops import knn_point, index_points
...
def sample_and_group(npoint, nsample, xyz, points):
    ...
    new_points = torch.concat([grouped_points_norm, new_points.view(B, S, 1, -1).repeat(1, 1, nsample, 1)], dim=-1)
...
class Point_Transformer2(nn.Module):
    ...
    def execute(self, x):
        ...
        x = torch.concat([x, feature_1], dim=1)
        x = self.conv_fuse(x)
        x = torch.max(x, 2)
...
class Point_Transformer(nn.Module):
    def __init__(self, output_channels=2048):
        ...
        # self.bn_my = nn.BatchNorm1d(2048)
        # self.bn6 = nn.BatchNorm1d(512)
        # self.dp1 = nn.Dropout(p=0.5)
        # self.linear2 = nn.Linear(512, 256)
        # self.bn7 = nn.BatchNorm1d(256)
...
if __name__ == '__main__':
    input_points = torch.randn((16, 3, 1024))  # B, D, N
