...
class weight_score(nn.Module):
    def __init__(self, batch_size, patch_number):
        super(weight_score, self).__init__()
        self.score_mlp = BasicFCModule(2048)
        self.weight_mlp = BasicFCModule(2048)
        self.batch_size = batch_size
        self.patch_number = patch_number

    def forward(self, fea_vector):
        """
...
class double_fusion(nn.Module):
    def __init__(self, batch_size, patch_number):
        ...
        self.score_compute = weight_score(batch_size, patch_number)
    ...
    def forward(self, fea_vector1, fea_vector2):
        score1 = self.score_compute(fea_vector1)
        score2 = self.score_compute(fea_vector2)
        # if np.any(np.isnan(score1.cpu().detach().numpy())) or np.any(np.isnan(score1.cpu().detach().numpy())):
        #     print("score is nan")
        x, difference = self.FusionLayer(score1, score2)  # B x 1
