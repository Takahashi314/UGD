...
def index_to_points(points, idx):
    ...
    S = idx.shape[1]
    C = points.shape[2]
    new_points = torch.zeros((B, S, C), dtype=torch.float32)
...
def fps_sampling_func(points, patch_num, patch_size, centroids=None):
    """
    对点云做fps采样
    :param points:      输入 N x 3 的点云        输入tenser  不用进gpu
    :param patch_num:   patch的数量
    :param patch_size:  每个patch中点的数量
    :return:            返回采样后的   patch_num x patch_size x 3     输入tenser  不用进gpu
    """
    # 对模型进行采样，取patch
    # start_time = time.time()
    points = points.reshape((1, -1, 3)).to(torch.float32)
    ...
    correct = (result[0, :, 0] == points.shape[1]).to(torch.int).clone().detach()
    # print(f"t1: {time.time() - start_time} points shape: {points.shape}")
    if torch.sum(correct):
        old_result = result
        true_indices = torch.nonzero(correct).squeeze(dim=1)
        # print('true_indices', true_indices)
        err_centroids = centroids[0, true_indices, :]
        try:
            for i in range(err_centroids.shape[0]):
                distances = torch.norm(points[0, :, :] - err_centroids[i, :], dim=1)
                nearest_point_index = torch.argmin(distances)
                nearest_point = points[0, nearest_point_index, :]
                centroids[0, true_indices[i], :] = nearest_point
                result = query_ball_point(0.2, patch_size, points, centroids)
    ...
    try:
        ...
        for patch in range(S):  # 0-64
            ...
            # print(patch_index)
            value = index_to_points(points, patch_index)  # value:[B patch_size C]
            for batch in range(B):
...
def index_points(points, idx):  # 将点集按照序号采样恢复成具体点坐标
    ...
    new_points = points[batch_indices, idx,
                 :]  # batch_indices与idx维度一致，batch_indices为[[0,0,0,0,0,...][1,1,1,,,,]....[B-1,B-1......] ]   idx为[[3,8,38,6,4..][....],[....]]
    return new_points
...
def farthest_point_sample(xyz, npoint):  # 最远点采样得到序号  BXS
    ...
    """
    device = xyz.device  # 1,num,3    n point 64
    B, N, C = xyz.shape
    centroids = torch.zeros(B, npoint, dtype=torch.long).to(device)  # 1*64
    distance = torch.ones(B, N).to(device) * 1e10  # 1 * num
    farthest = torch.randint(0, N, (B,), dtype=torch.long).to(device)  # 1 random num between 0-num
    batch_indices = torch.arange(B, dtype=torch.long).to(device)  # 0
    for i in range(npoint):  # 0-63
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    group_idx = group_idx.sort(dim=-1)[0][:, :,
                :nsample]  # B x S x nsample 先按照行排序，半径之外的点肯定排在最后，且只取了最大点数值,半径范围内如果达到点数，则取最大点数，否则取半径外点以达到最大点数
...
if __name__ == '__main__':
    ...
    print(points.shape)
    points = fps_sampling_func(points, 64, 256)
